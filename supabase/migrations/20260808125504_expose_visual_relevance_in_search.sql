-- Return the visual branch score so the application can keep visual evidence when the
-- text-only cross-encoder has nothing to work with. Without this, an image whose only
-- match is what it depicts is reranked to zero.
drop function if exists public.hybrid_search_files(uuid, text, vector, integer, integer, vector);
